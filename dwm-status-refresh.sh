...
print_disk() {
    disk=$(lsblk -f | grep sda1 | awk '{print $5}' | sed -r 's/G$//')
    echo -e "${disk}G"
}
...
if [ $monitor == connected ]; then
	xsetroot -name " $(print_wifi)  $(print_disk)  $(print_bat)  $(print_date) "
else
        xsetroot -name "  $(print_volume)  $(print_backlight)  $(print_wifi)  $(print_disk)  $(print_bat)  $(print_date) " 
fi
